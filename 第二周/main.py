...
def detect(opt):  # gradio可视化时需要加一个参数
    global count, count2, data, track_history
    count = 0
    count2 = 0
    data = []
    track_history = {}
...
def count_obj(box, w, h, id):
    ...
    if previous is None or track_id in data:
        return

    previous_x, previous_y = previous
    crossed_down = previous_y <= line_y < center_y
    if crossed_down:
        crossing_x = int((previous_x + center_x) / 2)
        if crossing_x > int(w / 2):
            count2 += 1
        else:
            count += 1
